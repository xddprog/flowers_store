...
from app.infrastructure.database.models.bouquet import (
    Bouquet,
    BouquetFlowerType,
    BouquetImage,
    BouquetType,
...
class BouquetRepository(SqlAlchemyRepository[Bouquet]):
    ...
    async def get_bouquet_types_with_bouquet_count(
        ...
        query = (
            select(BouquetType, func.count(Bouquet.id).label("bouquets_count"))
        ...
        if flower_type_ids:
            for flower_type_id in flower_type_ids:
                bouquet_flower_type = BouquetFlowerType(
                    bouquet_id=bouquet.id, flower_type_id=flower_type_id
                )
    ...
    async def increment_view_count(self, bouquet_id: str) -> None:
        query = select(Bouquet).where(Bouquet.id == bouquet_id)
        result = await self.session.execute(query)
    ...
    async def search_bouquets(
        ...
        if sort == BouquetSort.POPULAR:
            query = query.order_by(
                desc(Bouquet.purchase_count), desc(Bouquet.view_count)
            )
    ...
    async def update_image_order(
        self, bouquet_id: UUID, image_id: UUID, new_order: int
    ) -> list[BouquetImage]:
        images = (
            (
                await self.session.execute(
                    select(BouquetImage)
                    .where(BouquetImage.bouquet_id == bouquet_id)
    ...
    async def add_images(
        self, bouquet_id: UUID, image_paths: list[str]
    ) -> list[BouquetImage]:
        max_order_query = (
            select(BouquetImage.order)
            .where(BouquetImage.bouquet_id == bouquet_id)
            .order_by(BouquetImage.order.desc())
            .limit(1)
        )
        ...
        for index, image_path in enumerate(image_paths):
            image = BouquetImage(
                bouquet_id=bouquet_id, image_path=image_path, order=start_order + index
            )
    ...
    async def update_item(
        ...
        **update_values: Any
        ...
        # Удаляем flower_type_ids из update_values, так как это не колонка
        update_values.pop("flower_type_ids", None)

        # Обновляем основные поля букета (если есть что обновлять)
        bouquet = None
        if update_values:
            bouquet = await super().update_item(item_id, **update_values)
        else:
            # Если нет полей для обновления, просто получаем букет
            bouquet = await self.get_item(item_id)

        if not bouquet:
            return None

        # Если есть flower_type_ids, обновляем связи
        if flower_type_ids is not None:
            # Удаляем старые связи
            delete_query = select(BouquetFlowerType).where(
                BouquetFlowerType.bouquet_id == item_id
            )
            ...
            for flower_type_id in flower_type_ids:
                bouquet_flower_type = BouquetFlowerType(
                    bouquet_id=UUID(item_id), flower_type_id=flower_type_id
                )
                self.session.add(bouquet_flower_type)
    ...
    async def delete_image(self, bouquet_id: UUID, image_id: UUID) -> str | None:
    ...
    async def update_flower_types(
        self, bouquet_id: UUID, flower_type_ids: list[UUID]
    ) -> None:
        delete_query = select(BouquetFlowerType).where(
        ...
        for flower_type_id in unique_flower_type_ids:
            bouquet_flower_type = BouquetFlowerType(
                bouquet_id=bouquet_id, flower_type_id=flower_type_id
            )
            self.session.add(bouquet_flower_type)
